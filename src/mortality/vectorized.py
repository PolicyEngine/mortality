...
class VectorizedMortality:
    ...
    def adjust_for_characteristics(self,
                                  base_rates: np.ndarray,
                                  smoker: Optional[np.ndarray] = None,
                                  income_percentile: Optional[np.ndarray] = None,
                                  health_score: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if health_score is not None:
            ...
            health_effect = -0.3 * health_score  # Negative because good health reduces mortality
    ...
    def simulate_survival(self,
                         ...
                         improvement_rate: float = 0.01) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for year in range(n_years):
            ...
            still_alive = alive[:, year] if year > 0 else np.ones(self.n_sims, dtype=bool)
            ...
            death_ages = np.where(
                deaths_this_year,
                current_ages_alive + death_fractions,
                death_ages
            )
    ...
    def life_expectancy_vectorized(self,
                                   ages: np.ndarray,
                                   smoker: Optional[np.ndarray] = None,
                                   income_percentile: Optional[np.ndarray] = None,
                                   health_score: Optional[np.ndarray] = None,
