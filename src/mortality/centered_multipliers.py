...
class CenteredMultipliers:
    ...
    SMOKING = {
        ...
        "centered": {True: 1.61, False: 0.89},  # 1.8 / 1.12  # 1.0 / 1.12
    }
...
def demonstrate_centered_multipliers():
    ...
    smoking_prob = 0.25 - 0.002 * (
        income - 50
    )  # 25% at low income, 15% at median, 5% at high
    ...
    health = np.where(
        ...
        "excellent",
        np.where(
            health_scores > 0, "good", np.where(health_scores > -1, "average", "poor")
        ),
    )
    ...
    middle_good = np.mean(
        (health[(income >= 33) & (income < 67)] == "excellent")
        | (health[(income >= 33) & (income < 67)] == "good")
    )
    top_good = np.mean(
        (health[income >= 67] == "excellent") | (health[income >= 67] == "good")
    )
