...
class ConditionalMortalityModel:
    ...
    def calculate_population_average(cls) -> float:
        ...
        for quintile in range(1, 6):
            ...
            for health, health_prob in health_dist.items():
                mult = income_mult * cls.BASE_EFFECTS['health'][health] * cls.BASE_EFFECTS['smoking']
    ...
    @classmethod
    def get_calibrated_multiplier(cls,
                                  income_percentile: int,
                                  smoker: bool,
                                  health_status: str) -> float:
...
def demonstrate_conditional_model():
    ...
    # Smoking by income
    for q in range(1, 6):
        mask = (pop['income_percentile'] > (q-1)*20) & (pop['income_percentile'] <= q*20)
    ...
    for health in ['excellent', 'good', 'average', 'poor']:
        ...
        print(f"  {health}: {rate_smokers:.1%} (smokers) vs {rate_nonsmokers:.1%} (non-smokers)")
